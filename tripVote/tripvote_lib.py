...
def tripVote(myTree,refTrees, placement_taxa, W=None,do_indexing=True,search_space=None):
    myTree_obj = read_tree_newick(myTree)
    if W is None:
        W = [1.0]*len(refTrees)
    ...
    #converting placement_taxa to string
    placement_taxa_str = ""
    for t in placement_taxa:
        placement_taxa_str += t + ","
    # placement_taxa_str = placement_taxa_str[:-1]

    for rtree,w in zip(refTrees,W):
        mystr = triproot.tripRootScore(rtree, treestr, placement_taxa_str)
