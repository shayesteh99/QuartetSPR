...
bool RootedTree::pairAltWorld(RootedTree *t, unordered_map<string, bool> minus_one_leaves, bool do_pruning, TripletCounter *tripCount)
{
	//bool error = false;
	...
	for(vector<RootedTree*>::iterator i = l->begin(); i != l->end(); i++)
	{
		...
		// if (j == altWorldEnd && std::find(minus_one_leaves.begin(), minus_one_leaves.end(), leaf->name) == minus_one_leaves.end())
        if (j == altWorldEnd)
		{
			// This leaf wasn't found in the input tree!
            if (do_pruning || minus_one_leaves.find(leaf->name) == minus_one_leaves.end()){
