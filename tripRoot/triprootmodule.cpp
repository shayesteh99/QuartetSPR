...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD dwReason, LPVOID lpReserved) {
    ...
    bool get_tripScore(RootedTree *myTree, TripletCounter* myCount, double M, string &myScore){
        long long currScore = myCount->tripScore[myTree->idx]; // /M;
    ...
    static PyObject *method_tripRootScore(PyObject *self, PyObject *args) {
        const char *refnw, *treenw, *placement_taxa; // two newick strings and placement_taxa
        /* Parse arguments */
        if(!PyArg_ParseTuple(args, "sss", &refnw, &treenw, &placement_taxa)) {
            return NULL;
        }
        RootedTreeFactory *factory = new RootedTreeFactory();
        ...
        tripRoot.initialize(refTree, myTree, placement_taxa);
	PyObject* result;
