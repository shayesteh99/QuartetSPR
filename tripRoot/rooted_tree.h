...
class RootedTree
{
	public:
        ...
        double edge_length;
        double d2root;
        // vector<string> minus_one_leaves; // added by shayesteh99
		...
		bool pairAltWorld(RootedTree *t, unordered_map<string, bool> minus_one_leaves, bool do_pruning=true, TripletCounter *tripCount=NULL);
		void colorSubtree(int c);
		void markHDTAlternative();
		bool isError();
		RootedTree *contract(RootedTreeFactory *factory = NULL);
		// int get_n_minus_one_leaves();
